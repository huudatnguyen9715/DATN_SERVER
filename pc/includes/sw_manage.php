        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <?php
                    ...
                    echo '<td>';
                        echo '<button id="pin_'.$row['pin'].'_1" class="btn btn-sm btn-danger '; if($row['status'] == 1) echo "hide"; echo '"role="button" onclick="onOff(this.id)"=>Deivce Off</button>';
                        echo '<button id="pin_'.$row['pin'].'_0"class="btn btn-sm btn-success '; if($row['status'] == 0) echo "hide"; echo '"role="button" onclick="onOff(this.id)">Device On</button>';
                    echo '</td>';
                    echo "<tr>";
                    $i+=1;
                    endwhile;
...
<script>
    ...
    */
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for(i=0; i<vals.length-1; i++){
                    ...
                    var selec_off = "pin_"+temp_array[0] + "_0";
                    var selec_on = "pin_"+temp_array[0] + "_1";
    ...
    function onOff(id) {
        ...
        var url = "ajax/pin_status.php?"+ temp_array[0] + "_" + temp_array[1] + "=" + temp_array[2];
        console.log(url);
        $.ajax({
    ...
    // EXAMPLE
    /*
    $('#pin_27_on').click(function(){
        var $btn = $(this);
        $btn.button('loading');
        $.ajax({url: "ajax/pin_status.php?pin_27=1", success: function(result){
            console.log("-----PIN 27 ON!")
        }});
        setTimeout(
            function() {
               $btn.button('reset');
